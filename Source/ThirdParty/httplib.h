...
DECLARE_LOG_CATEGORY_CLASS(LogHttpLib, Log, All);
...
inline FString GetWinSockError()
{
    FString Err;
#ifdef _WIN32
    wchar_t* s = NULL;
    FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
        ...
        (LPWSTR)&s, 0, NULL);
    Err = WCHAR_TO_TCHAR(s);
    LocalFree(s);
    if (Err.IsEmpty())
    {
        return FString::Printf(TEXT("Unknown error code %d"), WSAGetLastError());
    }
#endif
    return Err;
}
...
socket_t create_socket(const char *host, int port, int socket_flags,
  ...
  if (getaddrinfo(host, service.c_str(), &hints, &result)) {
    ...
    res_init();
#endif
    UE_LOG(LogHttpLib, Error, TEXT("getaddrinfo failed: %s"), *GetWinSockError());
  ...
  for (auto rp = result; rp; rp = rp->ai_next) {
    ...
    if (sock == INVALID_SOCKET) {
      UE_LOG(LogHttpLib, Error, TEXT("WSASocketW failed: %s"), *GetWinSockError());
...
Server::create_server_socket(const char *host, int port, int socket_flags,
  ...
  return detail::create_socket(
      ...
      [](socket_t sock, struct addrinfo &ai) -> bool {
        if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) {
          UE_LOG(LogHttpLib, Error, TEXT("bind failed: %s"), *GetWinSockError());
          return false;
        }
        if (::listen(sock, 5)) { // Listen through 5 channels
          UE_LOG(LogHttpLib, Error, TEXT("listen failed: %s"), *GetWinSockError());
          return false;
        }
        return true;
      });
...
inline int Server::bind_internal(const char *host, int port, int socket_flags) {
  ...
  if (svr_sock_ == INVALID_SOCKET) { 
    UE_LOG(LogHttpLib, Error, TEXT("create_server_socket failed."));
    return -1; 
  }
  ...
  if (port == 0) {
    ...
    if (getsockname(svr_sock_, reinterpret_cast<struct sockaddr *>(&addr),
                    &addr_len) == -1) {
        UE_LOG(LogHttpLib, Error, TEXT("getsockname failed: %s"), *GetWinSockError());
      return -1;
    }
    if (addr.ss_family == AF_INET) {
      ...
      return ntohs(reinterpret_cast<struct sockaddr_in6 *>(&addr)->sin6_port);
        ...
        UE_LOG(LogHttpLib, Error, TEXT("Invalid ss_family: %d"), addr.ss_family);
      return -1;
    }
  } else {
    return port;
...
::SSLServer(const char *cert_path, const char *private_key_path,
                            ...
                            const char *client_ca_cert_dir_path, int* out_err) {
  ...
  if (ctx_) {
    ...
    int ret;
    if ((ret = SSL_CTX_use_certificate_chain_file(ctx_, cert_path)) != 1) {
        SSL_CTX_free(ctx_);
        ctx_ = nullptr;
       *out_err = 2;
    }
    else if ((ret = SSL_CTX_use_PrivateKey_file(ctx_, private_key_path, SSL_FILETYPE_PEM)) != 1) {
      SSL_CTX_free(ctx_);
      ctx_ = nullptr;
      *out_err = 3;

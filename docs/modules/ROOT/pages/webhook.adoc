...
Webhook notifications are a powerful way to get real-time updates on various in-game events. These notifications can be sent to platforms like Discord, Slack, or any service that supports webhooks. The mod provides a set of default JSON templates for different types of notifications, which can be customized to fit your needs.

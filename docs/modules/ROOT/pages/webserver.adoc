...
FRM provides a HTTP & WebSocket server that can be configured on a port of your choosing (Default: 8080). This can be modified by changing the respective configuration value in either the Server Manager for Dedicated Servers or the Main Menu Options for Single-Player, Hosted Sessions, and Client Sessions.

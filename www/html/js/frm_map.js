window.onload = function () {
  ...
  setTimeout(() => {
    if (playerLocation) map.setView(playerLocation, -6);
  }, 1500);
  ...
  function createRealtimeLayer(url, container = null) {
    return L.realtime(url, {
      ...
      updateFeature: function (feature, marker) {
        ...
        let popup = `<h3>${
          TypeName[feature.properties.type]
        }</h3><p>Location: ${x.toFixed(2)}, ${y.toFixed(2)}, ${z.toFixed(
          2
        )}</p>`;
        ...
        const speed =
          Math.sqrt(
            Math.pow(lastpos[0] - x, 2) +
              Math.pow(lastpos[1] - y, 2) +
              Math.pow(lastpos[2] - z, 2)
          ) / elapsed;

...
function displayFactory(getFactory) {
    ...
    for (i in getFactory) {
        ...
        for (p in getFactory[i].production) {
            prod += "Output: " + getFactory[i].production[p].Name + " / " + getFactory[i].production[p].CurrentProd.toFixed(2) + "<br>";
        };

        for (n in getFactory[i].ingredients) {
            need += "Output: " + getFactory[i].ingredients[n].Name + " / " + getFactory[i].ingredients[n].CurrentConsumed.toFixed(2) + "<br>";
        };
